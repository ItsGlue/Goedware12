...
public class ShieldEnemy : MonoBehaviour
{
    ...
    void Die()
    {
        if (Input.GetKeyDown(KeyCode.Space)) // For example, use the space key to activate
        {
            ...
            if (player != null)
            {
                ...
                AutoDeactivate autoDeactivateScript = player.GetComponent<AutoDeactivate>();

                if (autoDeactivateScript != null)
                {
                    // Call the method on the AutoDeactivate script
                    autoDeactivateScript.ActivateAndDeactivate(3f); // Activate for 3 seconds
                }
                else
                {
