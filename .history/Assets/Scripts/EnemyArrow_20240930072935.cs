...
public class EnemyArrow : MonoBehaviour
{
     ...
     private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("Player") || collision.gameObject.CompareTag("))
        {
...
{
    // Check if the collider is not the Player
    if (collision.gameObject.CompareTag("Player") || collision.gameObject.CompareTag("Sheild"))
    {
        Destroy(gameObject); // Destroy the arrow upon collision with anything that's not the Player
        PlayerController playerController = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerController>();
            if (playerController != null)
