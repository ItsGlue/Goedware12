    ...
    private void OnTriggerEnter2D(Collider2D collision)
{
    if (collision.CompareTag("Enemy") || collision.CompareTag("SwordEnemy"))
    {
        ...
        if (enemyRigidbody != null)
        {
            ...
            // Zero out the enemy's velocity before applying knockback
...
private void ApplyKnockback(Rigidbody2D enemyRigidbody, Vector2 knockbackDirection)
{
    enemyRigidbody.velocity = Vector2.zero;  // Reset velocity
    enemyRigidbody.AddForce(knockbackDirection * knockbackForce, ForceMode2D.Impulse);
}
